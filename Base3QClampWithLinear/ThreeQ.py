...
class Network(nn.Module):
        ...
        for _ in range(n_steps):
            ...
            with torch.enable_grad():
                if beta == 0.0:
                    ...
                    output = (1 - beta) * layers[-1] + beta * y_one_hot
                    F_sum = self.energy(layers[:-1] + [output]).sum()
    ...
    def train_batch(
        ...
        output = (1 - self.beta) * layers_beta_det[-1] + self.beta * y_one_hot
        F_beta = self.energy(layers_beta_det[:-1] + [output]).mean()
