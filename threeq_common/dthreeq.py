...
class DThreeQMLP(nn.Module):
    ...
    def energy(self, layers: List[torch.Tensor]) -> torch.Tensor:
        ...
        for residual in self.residuals(layers):
            energy = energy + 0.5 * residual.pow(2).sum(dim=1)
        return energy
    ...
    def apply_target(
        ...
        if mode.kind == "direct":
            target = y_one_hot if sign >= 0 else -y_one_hot
            return target
        if mode.kind == "nudge":
            signed_beta = sign * mode.beta
            return (1.0 - signed_beta) * output + signed_beta * y_one_hot
        if mode.kind == "gradual":
            signed_gamma = sign * mode.gamma
            return (1.0 - signed_gamma) * output + signed_gamma * y_one_hot
    ...
    def dplus_objective(
        ...
        terms = []
        for r_clamped, r_free in zip(residuals_clamped, residuals_free):
            terms.append(0.5 * (r_clamped - r_free).pow(2).mean())
    ...
    def ep_objective(
        ...
        beta_scale = 1.0
        if self.target_mode.kind == "nudge":
    ...
    def eval_batch(self, x: torch.Tensor, y: torch.Tensor) -> Dict[str, float]:
        ...
        with torch.no_grad():
            ...
            return {
                ...
                "test_cost": F.mse_loss(states_free[-1], y_one_hot).item(),
