...
为便于论文撰写，本轮新增 `paper_assets/` 作为统一入口。该目录不重新训练模型，只读取已有 `experiments/*/results` 和少量 legacy 收敛性证据，生成 8 张核心图、5 组 Markdown/CSV 表，以及中文论文材料摘要：
...
| 2026-04-19 | this commit | 新增 ThreeQ convergence/divergence LaTeX report；补充 $\rho>1$ 受控 inference 发散与大步训练跨越稳定边界后的 post-update divergence 图表 |
