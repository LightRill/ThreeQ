...
最后更新：2026-04-18  
当前状态：第一轮理论整理、最小可比较实验、DThreeQ 可行性筛查、DThreeQ 机制诊断、Dplus 修正诊断、MNIST 小预算压力筛查、legacy MNIST 复现实验、DThreeQ MNIST focused/longrun 筛查、EPThreeQ 调参、DThreeQ objective/activation/prediction audit 已完成。
...
DThreeQ 需要分成两部分判断：EP-nudge 版本已经能在 MNIST 上学习到约 84% accuracy，短暂 early-best 可到约 84.7%，而当前 Dplus residual-delta 目标仍不能作为主训练规则直接扩大 benchmark。它的失败不是简单的“不能下降”，而是：
...
MNIST focused/longrun 与后续 accuracy audits 进一步说明：EP-nudge 能把 DThreeQ 从随机水平拉到 0.16 test error，但 direct Dplus 即使 state delta 达到 0.16 到 0.18，分类仍接近随机；plus-energy、target matching、`sigmoid`、`relu`、`clip01`、线性边预测和 activated initialization 也没有突破该平台。因此下一步不应继续堆叠标量缩放或简单改激活，而应核对外部 90%+ DThreeQ 的目标函数，并优先测试 input-energy normalization/removal、logit CE nudging、softmax/readout calibration、signed residual target、输出到隐藏层的显式监督注入等结构性修改。
...
11. **核对外部 90%+ DThreeQ 定义。** 当前实现的 EP-nudge 稳定最好约 84%，高学习率 early-best 约 84.7%，Dplus direct、plus-energy、target matching、`clip01` 和线性边预测均失败；如果外部 DThreeQ 能过 90%，最优先要对齐的是 objective、label encoding、input energy weighting、optimizer、是否使用 feedforward CE/readout，以及是否保存 best checkpoint，而不是继续只加 epoch。
