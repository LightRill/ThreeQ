...
最后更新：2026-04-18  
当前状态：第一轮理论整理、最小可比较实验、DThreeQ 可行性筛查、DThreeQ 机制诊断已完成。
...
4. **当前 DThreeQ/Dplus 规则没有通过机制诊断。** Dplus 与 BP 的 forward-only 更新方向几乎不对齐，cosine 仅约 0.03 到 0.08；Dplus 与 EP 在 one-sided targets 上高度同向，但 norm ratio 极小，说明它更像“尺度塌缩的 EP-like 更新”，不是 BP-like 更新。

因此，短期主线不应直接扩大到 MNIST/FashionMNIST，而应先修正 Dplus 更新方向和尺度，并建立每次实验都可比较的日志、图表和诊断。
...
| 公共实现 | `threeq_common/` | 可比较 MLP/DThreeQ/机制诊断实现 |
| radas 实验 | `experiments/minimal_suite/`、`experiments/dthreeq_suite/`、`experiments/mechanism_diagnostic/`、`experiments/analysis_summary/` | 可复现的参数矩阵、结果物化、图表和汇总 |
...
### 5.4 Legacy CNN / Decision Boundary Figures
...
DThreeQ 的 Dplus 目标目前没有直接可用。它的失败不是简单的“不能下降”，而是：

- 与 BP 更新方向不对齐；
- 与 EP 同向但尺度塌缩；
- plusminus 会产生符号抵消；
- raw 训练步下 Dplus 更新弱到不足以驱动表示学习。

因此下一步应先修改 Dplus 规则，而不是扩大 benchmark。

## 7. 下一步研究计划

### 7.1 Dplus 修正优先级

优先尝试以下修改，并用 `experiments/mechanism_diagnostic/` 重复诊断：

1. **Residual normalization**：对 $r_\beta-r_0$ 按 residual norm 或 layer dimension 归一化，避免目标尺度过小。
2. **EP-scale calibration**：让 Dplus 更新范数与 BP 或 EP 的 forward 更新范数同量级，再比较方向。
3. **禁用 plusminus 主线**：当前 plusminus 明确抵消，先只保留 plus。
4. **Layer-wise gain**：为不同层 residual delta 加可学习或固定 gain，避免输出层扰动无法传到浅层。
5. **Direction regularizer**：在机制诊断中加入 one-step loss decrease 约束，筛掉不会下降的目标构造。

通过标准：

- `dplus_vs_bp` forward cosine 明显高于当前 0.03 到 0.08 区间；
- Dplus raw one-step free-MSE decrease 达到 BP 同量级；
- positive rate 在多个 seed 下接近 1；
- 再回到 two moons screen 时，Dplus mean best test error 至少接近 EP/BP 控制组。
