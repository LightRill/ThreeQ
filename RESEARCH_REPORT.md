...
最后更新：2026-04-18  
当前状态：第一轮理论整理、最小可比较实验、DThreeQ 可行性筛查、DThreeQ 机制诊断、Dplus 修正诊断、MNIST 小预算压力筛查、legacy MNIST 复现实验、DThreeQ MNIST focused/longrun 筛查已完成。
...
8. **当前 DThreeQ MNIST 实现尚未复现 90%+，但 EP-nudge 路线明显有效。** 在 10k train / 2k test / 12 epoch focused screen 中，`dthreeq_ep_nudge0p1_lr1e3` 达到 0.1965 mean best test error；继续到 30 epoch 后，`dthreeq_ep_nudge0p1_lr3e3` 达到 0.1606，约 83.9% accuracy。Dplus direct/layergain 仍接近随机，说明如果外部 DThreeQ 能到 90%+，很可能使用了不同目标函数、初始化、监督注入或完整数据设置。
...
| radas 实验 | `experiments/minimal_suite/`、`experiments/dthreeq_suite/`、`experiments/mechanism_diagnostic/`、`experiments/dplus_fix_diagnostic/`、`experiments/mnist_suite/`、`experiments/mnist_legacy_repro/`、`experiments/mnist_dthreeq_focus/`、`experiments/mnist_dthreeq_longrun/`、`experiments/analysis_summary/` | 可复现的参数矩阵、结果物化、图表和汇总 |
...
DThreeQ 需要分成两部分判断：EP-nudge 版本已经能在 MNIST 上学习到约 84% accuracy，而当前 Dplus residual-delta 目标仍不能作为主训练规则直接扩大 benchmark。它的失败不是简单的“不能下降”，而是：
...
MNIST focused/longrun 进一步说明：EP-nudge 能把 DThreeQ 从随机水平拉到 0.16 test error，但 direct Dplus 即使 state delta 达到 0.16 到 0.18，分类仍接近随机。因此下一步不应继续堆叠标量缩放，而应核对外部 90%+ DThreeQ 的目标函数，并测试 plus-energy、local target-prop MSE、signed residual target、输出到隐藏层的显式监督注入等结构性修改。
...
1. **完整复现 legacy MNIST。** 先把 `EPBase3Q` 和 `Base3Q` 从 5k/1k、8 epoch 扩展到原始 50k/10k、25 epoch，并至少跑 3 个 seed；这是验证 85% accuracy 观察的主线。
...
4. **去掉输入重构主导项。** 当前能量把 784 维输入层也放进双向预测误差，高维 input reconstruction 很容易压过 10 维分类监督。下一版 MNIST MLP/CNN 应把输入作为 clamped boundary，只对隐藏层和输出层建能量，或者对输入项加 $1/784$ 量级归一化。
5. **改输出监督目标。** 当前输出层多为 MSE one-hot state；MNIST 上应比较 logits + cross-entropy nudging、softmax state、label smoothing，以及只 clamp output residual 而不强行替换整个输出状态。
...
11. **核对外部 90%+ DThreeQ 定义。** 当前实现的 EP-nudge 最好约 84%，Dplus direct 失败；如果外部 DThreeQ 能过 90%，最优先要对齐的是 objective、label encoding、state initialization、optimizer 和是否使用 feedforward CE/readout，而不是继续只加 epoch。
