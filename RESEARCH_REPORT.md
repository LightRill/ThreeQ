...
最后更新：2026-04-18  
当前状态：第一轮理论整理、最小可比较实验、DThreeQ 可行性筛查、DThreeQ 机制诊断、Dplus 修正诊断、MNIST 小规模可比较筛查已完成。
...
6. **MNIST 小规模筛查确认当前结构没有跨到高维视觉任务。** 在 3000 train / 1000 test / 3 epoch 的 MNIST screen 中，BP MLP 与 BP CNN 的 mean best test error 分别为 0.1114 和 0.1188；而 ThreeQ/EPThreeQ/DThreeQ/Dplus/CNNThreeQ/EPCNNThreeQ 均停在 0.8668 到 0.9163，接近十分类随机水平。
...
| radas 实验 | `experiments/minimal_suite/`、`experiments/dthreeq_suite/`、`experiments/mechanism_diagnostic/`、`experiments/dplus_fix_diagnostic/`、`experiments/mnist_suite/`、`experiments/analysis_summary/` | 可复现的参数矩阵、结果物化、图表和汇总 |
...
### 5.6 MNIST Comparable Screen

实验路径：`experiments/mnist_suite/`
结果路径：`experiments/mnist_suite/results/`

这轮不是完整 MNIST benchmark，而是为了回答一个更基础的问题：在相同小规模 MNIST 子集上，当前 ThreeQ 系列能否明显离开随机水平。配置为 3000 个训练样本、1000 个测试样本、3 个 epoch、2 个 seed，并同时包含 BP MLP/BP CNN 作为 sanity-check baseline。
...
因此，在当前更新规则下，直接扩大 MNIST 训练轮数的研究价值有限。下一步应先解决高维输入能量支配、状态初始化、输出监督注入和 BP-like direction alignment。
...
MNIST screen 的下一步不应只加 epoch，而应按以下顺序拆解：

1. **去掉输入重构主导项。** 当前能量把 784 维输入层也放进双向预测误差，高维 input reconstruction 很容易压过 10 维分类监督。下一版 MNIST MLP/CNN 应把输入作为 clamped boundary，只对隐藏层和输出层建能量，或者对输入项加 $1/784$ 量级归一化。
2. **使用 feedforward/amortized state initialization。** 当前很多模型从零状态开始 relaxation，MNIST 上自由相在短步数内难以形成好表示。应使用前向 pass 初始化 hidden/output state，再做少量局部 relaxation。
3. **改输出监督目标。** 当前输出层多为 MSE one-hot state；MNIST 上应比较 logits + cross-entropy nudging、softmax state、label smoothing，以及只 clamp output residual 而不强行替换整个输出状态。
4. **控制 hard clipping 饱和。** ThreeQ MLP 的 saturation 约 0.38 到 0.45，说明稳定性来自大量状态卡边界。可用 tanh/softsign、layer norm、spectral norm 或 learnable damping 替代纯 clipping。
5. **先做 MNIST mechanism diagnostic。** 在同一 MNIST mini-batch 上重复 Dplus/BP/EP cosine、norm ratio、sign agreement、one-step CE decrease，而不是直接用 two-moons 的机制结论外推。
6. **分阶段训练。** 先训练 BP forward encoder 到可用表示，再冻结/半冻结 encoder 测试 EP/Dplus 局部更新是否能维持或微调；这能区分“表示学不出来”和“局部规则无法维护表示”。
7. **CNNThreeQ 重新设计能量粒度。** 卷积/反卷积对称是合理结构，但不应让所有像素 reconstruction 与分类项同权。应采用 patch/block energy、pooling state、channel-wise normalization，以及只在高层使用 supervised nudging。
8. **Dplus 结构性目标继续优先。** MNIST 结果进一步支持前一节判断：layer gain 和 residual norm 不是方向解法。下一版应实现 signed residual target 与 output-to-hidden residual injection，然后先过 MNIST mechanism gate，再做训练 sweep。
