...
class ThreeQ(nn.Module):
    def __init__(
        ...
        if use_xavier:
            w_init = torch.empty((n, n), device=self.device)
            nn.init.xavier_uniform_(w_init)
        else:
            w_init = torch.randn((n, n), device=self.device)
    ...
    def compute_rho(self) -> float:
        ...
        if not torch.isfinite(J_ff).all():
            return float("nan")
    ...
    def fix(
        ...
        inf_log_every: int = 1,
        log_path: str | Path | None = None,
        inf_log_dir: str | Path | None = None,
        plot_path: str | Path | None = None,
        inf_plot_dir: str | Path | None = None,
    ):
        ...
        if log_path is not None:
            ...
            writer.writerow(["# num_epochs", int(num_epochs)])
            writer.writerow(["# log_every", int(log_every)])
            writer.writerow(["# inf_num_epochs", int(inf_num_epochs)])
            writer.writerow(["# inf_lr", float(inf_lr)])
            writer.writerow(["# inf_log_every", int(inf_log_every)])

        if inf_log_dir is not None:
            inf_log_dir = Path(inf_log_dir)
            inf_log_dir.mkdir(parents=True, exist_ok=True)
        if inf_plot_dir is not None:
            inf_plot_dir = Path(inf_plot_dir)
            inf_plot_dir.mkdir(parents=True, exist_ok=True)
        inf_log_every = max(1, int(inf_log_every))
        inf_step_log_every = max(1, int(inf_num_epochs) // 100)

        for epoch in range(1, 1 + num_epochs):
            inf_log_path = None
            should_log_inf = epoch % inf_log_every == 0
            if should_log_inf and inf_log_dir is not None:
                inf_log_path = inf_log_dir / f"inf_epoch_{epoch:04d}.csv"
            self.inference(
                inf_num_epochs,
                inf_lr,
                log_path=inf_log_path,
                log_every=inf_step_log_every,
            )
            if should_log_inf and inf_log_path is not None and inf_plot_dir is not None:
                plot_energy(
                    inf_log_path,
                    out_path=inf_plot_dir / f"inf_epoch_{epoch:04d}.png",
                )
            E = self.energy()
            optimizer.zero_grad()
            E.backward()
            ...
            if epoch % log_every == 0 and writer is not None:
                e_value = float(self.energy().detach().cpu())
                rho_value = self.compute_rho()
                writer.writerow([epoch, e_value, rho_value])

        if log_file is not None:
            log_file.close()
        if log_path is not None and plot_path is not None:
            plot_energy(log_path, out_path=plot_path)
...
def plot_energy(
    ...
    with log_path.open("r", newline="") as f:
        ...
        if header is not None:
            ...
            if "delta" in columns:
                delta_idx = columns.index("delta")
            tracked_names = [
                name
                for name in columns
                if name not in ("epoch", "E", "delta", "rho") and name != ""
            ]
            tracked_indices = [columns.index(name) for name in tracked_names]
        ...
        for row in reader:
            ...
            if delta_idx is not None and len(row) > delta_idx and row[delta_idx] != "":
                deltas.append(float(row[delta_idx]))
            if rho_idx is not None and len(row) > rho_idx and row[rho_idx] != "":
                rhos.append(float(row[rho_idx]))
            for name, idx in zip(tracked_names, tracked_indices):
    ...
    if rhos and len(rhos) == len(epochs):
        ...
        ax_r.ticklabel_format(style="plain", axis="y", useOffset=False)
